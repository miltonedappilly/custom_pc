...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 80; i++) {
      particles.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, vx: (Math.random() - 0.5) * 0.4, vy: (Math.random() - 0.5) * 0.4, size: Math.random() * 1.5 + 0.5, opacity: Math.random() * 0.5 + 0.1 });
    }
    let frame: number;
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 120) {
            ...
            ctx.strokeStyle = `rgba(79, 142, 247, ${0.12 * (1 - dist / 120)})`; ctx.lineWidth = 0.5; ctx.stroke();
          }
        }
      }
  ...
  return (
    <section id="home" className="relative min-h-screen flex flex-col justify-center overflow-hidden mesh-bg">
      ...
      {floatingIcons.map(({ icon: Icon, label, delay, x, y }) => (
        <div key={label} className="absolute hidden lg:flex flex-col items-center gap-2 animate-float pointer-events-none" style={{ left: x, top: y, animationDelay: delay }}>
          <div className="glass w-14 h-14 rounded-2xl flex items-center justify-center border border-white/10"><Icon className="w-6 h-6 text-blue-400" /></div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pt-28 pb-20">
        <div className="max-w-3xl">
          ...
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.08] tracking-tight mb-6">
            <span className="text-white">Power Your</span><br />
            <span className="shimmer-text">Digital Future</span><br />
          ...
          <div className="flex flex-wrap gap-4">
            <button onClick={() => scrollTo("products")} className="btn-primary px-7 py-4 text-base">Explore Products <ArrowRight className="w-5 h-5" /></button>
            <button onClick={() => scrollTo("contact")} className="btn-ghost px-7 py-4 text-base">Talk to an Expert</button>
          </div>
        </div>
      ...
      <button onClick={() => scrollTo("products")} className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 text-white/30 hover:text-white/60 transition-colors animate-bounce">
